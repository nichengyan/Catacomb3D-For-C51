...
/*
{
        ...
        const uint8_t ParticleSystem = 1;
};
*/
class AnchorType
{
...
/*
{
	...
	const uint8_t BelowCenter=2;
	const uint8_t Ceiling=3;
};
*/
struct QueuedDrawable
{
	union
	{
        ...
        DrawableType::Value type;//x DrawableType type : 1;
	bool invert;//x bool invert : 1;
	int8_t x;
	int8_t y;
	uint8_t halfSize;
...
public:
	...
	static void DrawObject(const uint16_t* spriteData, int16_t x, int16_t y, uint8_t scale = 128, AnchorType::Value anchor = AnchorType::Floor, bool invert = false);
