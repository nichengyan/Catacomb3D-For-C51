 ...
 const EnemyArchetype Enemy::archetypes[(int)EnemyType::NumEnemyTypes] PROGMEM =
{
	{
		// Skeleton
		(const uint16_t*)skeletonSpriteData,
		50,					// hp
		4,					// speed
		20,					// attackStrength
		3,					// attackDuration
	...
	{
		// Spider
		(const uint16_t*)spiderSpriteData,
		10,					// hp
		7,					// speed
		5,					// attackStrength
		1,					// attackDuration
...
void Enemy::Init(EnemyType::Value initType, int16_t initX, int16_t initY)
{
...
const EnemyArchetype* Enemy::GetArchetype() const
{
	if (type == EnemyType::None)
		return nullptr;
	return (EnemyArchetype const*) &archetypes[(int)type];
}
...
void EnemyManager::Spawn(EnemyType::Value enemyType, int16_t x, int16_t y)
{
        for (uint8_t n = 0;n < EnemyManager::maxEnemies; n++)
        {
            Enemy &enemy = EnemyManager::enemies[n];
...
void EnemyManager::SpawnEnemies()
{
	for (uint8_t y = 0; y < Map::height; y++)
	{
		for (uint8_t x = 0; x < Map::width; x++)
		{
			...
			{
                            EnemyType::Value type = (EnemyType::Value)((Random() % ((int)(EnemyType::NumEnemyTypes))));
