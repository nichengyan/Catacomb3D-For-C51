...
Projectile* ProjectileManager::FireProjectile(Entity* owner, int16_t x, int16_t y, uint8_t angle)
{
        ...
        {
                Projectile &p = ProjectileManager::projectiles[n];   
		if(p.life == 0)
		{
			...
			{
				for (uint8_t m = 0; m < EnemyManager::maxEnemies; m++)
				{
					if (&EnemyManager::enemies[m] == owner)
					{
						p.ownerId = m;
						break;
					}
...
void ProjectileManager::Draw()
{
		...
		if (p.life > 0)
		{
			Renderer::DrawObject((uint16_t const*)(p.ownerId == Projectile::playerOwnerId ? projectileSpriteData : enemyProjectileSpriteData), p.x, p.y, 32, AnchorType::BelowCenter);
		}
	}
}
