...
void Player::Tick()
{
	...
	case CellType::Potion:
		if (hp < maxHP)
		{
			...
			Game::ShowMessage((char const*)PSTR("Drank a potion of healing"));
		}
		break;
	case CellType::Coins:
		Map::SetCell(cellX, cellY, CellType::Empty);
		Platform::PlaySound(Sounds::Pickup);
		Game::ShowMessage((char const*)PSTR("Found some gold coins"));
		Game::stats.coinsCollected++;
		break;
	case CellType::Crown:
		Map::SetCell(cellX, cellY, CellType::Empty);
		Platform::PlaySound(Sounds::Pickup);
		Game::ShowMessage((char const*)PSTR("Found a jewel encrusted crown"));
		Game::stats.crownsCollected++;
		break;
	case CellType::Scroll:
		Map::SetCell(cellX, cellY, CellType::Empty);
		Platform::PlaySound(Sounds::Pickup);
		Game::ShowMessage((char const*)PSTR("Found an ancient scroll"));
...
bool Player::CheckCollisions()
{
	...
	uint8_t lookAheadCellY = (uint8_t)(lookAheadY / CELL_SIZE);
        ...
        CellType::Value lookAheadCell = Map::GetCellSafe(lookAheadCellX, lookAheadCellY);
	...
	case CellType::Chest:
		...
		Platform::PlaySound(Sounds::Pickup);
		Game::ShowMessage((char const*)PSTR("Found a chest full of treasure!"));
		Game::stats.chestsOpened++;
		break;
	case CellType::Sign:
		Game::ShowMessage((char const*)SignMessage1);
		break;
	}
