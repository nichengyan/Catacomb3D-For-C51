...
void Menu::Draw()
{
	...
	Font::PrintString((char const*)PSTR("CATACOMBS OF THE DAMNED"), 2, 18, COLOUR_WHITE);
	Font::PrintString((char const*)PSTR("by @jameshhoward"), 7, 32, COLOUR_WHITE);
	Font::PrintString((char const*)PSTR("Start"), 4, 24, COLOUR_WHITE);
	Font::PrintString((char const*)PSTR("Sound:"), 5, 24, COLOUR_WHITE);

	if (Platform::IsAudioEnabled())
	{
		Font::PrintString((char const*)PSTR("on"), 5, 52, COLOUR_WHITE);
	}
	else
	{
		Font::PrintString((char const*)PSTR("off"), 5, 52, COLOUR_WHITE);
	}
	
	Font::PrintString((char const*)PSTR(">"), 4 + selection, 16, COLOUR_WHITE);
	
	const uint16_t* torchSprite = (const uint16_t*) (Game::globalTickFrame & 4 ? torchSpriteData1 : torchSpriteData2);
	Renderer::DrawScaled(torchSprite , 0, 10, 9, 255,false);
	Renderer::DrawScaled(torchSprite , DISPLAY_WIDTH - 18, 10, 9, 255,false);
}
...
{
	Platform::FillScreen(COLOUR_BLACK);
	Font::PrintString((char const*)PSTR("Entering floor"), 3, 30, COLOUR_WHITE);
...
void Menu::DrawGameOver()
{
	...
	Platform::FillScreen(COLOUR_BLACK); 
	Font::PrintString((char const*)PSTR("GAME OVER"), 0, 64 - 18, COLOUR_WHITE);

	switch (Game::stats.killedBy)
	{
	case EnemyType::None:
		Font::PrintString((char const*)PSTR("You escaped the catacombs!"), 1, 12, COLOUR_WHITE);
		break;
	case EnemyType::Mage:
		Font::PrintString((char const*)PSTR("Killed by a mage on level"), 1, 8, COLOUR_WHITE);
		Font::PrintInt(Game::floor, 1, 112, COLOUR_WHITE);
		break;
	case EnemyType::Skeleton:
		Font::PrintString((char const*)PSTR("Killed by a knight on level"), 1, 4, COLOUR_WHITE);
		Font::PrintInt(Game::floor, 1, 116, COLOUR_WHITE);
		break;
	case EnemyType::Bat:
		Font::PrintString((char const*)PSTR("Killed by a bat on level"), 1, 10, COLOUR_WHITE);
		Font::PrintInt(Game::floor, 1, 110, COLOUR_WHITE);
		break;
	case EnemyType::Spider:
		Font::PrintString((char const*)PSTR("Killed by a spider on level"), 1, 4, COLOUR_WHITE);
		Font::PrintInt(Game::floor, 1, 116, COLOUR_WHITE);
		break;
	}
	
	Font::PrintString((char const*)PSTR("LOOT:"), 2, 20, COLOUR_WHITE);

	constexpr uint8_t firstRow = 21;
	constexpr uint8_t secondRow = 38;

	Renderer::DrawScaled((uint16_t const*)chestSpriteData, 0, firstRow, 9, 255);
	Font::PrintInt(Game::stats.chestsOpened, 4, 18, COLOUR_WHITE);

	Renderer::DrawScaled((uint16_t const*)crownSpriteData, 0, secondRow, 9, 255);
	Font::PrintInt(Game::stats.crownsCollected, 6, 18, COLOUR_WHITE);

	Renderer::DrawScaled((uint16_t const*)scrollSpriteData, 30, firstRow, 9, 255);
	Font::PrintInt(Game::stats.scrollsCollected, 4, 48, COLOUR_WHITE);

	Renderer::DrawScaled((uint16_t const*)coinsSpriteData, 30, secondRow, 9, 255);
	Font::PrintInt(Game::stats.coinsCollected, 6, 48, COLOUR_WHITE);

	///
	int offset = (Game::globalTickFrame & 8) == 0 ? 32 : 0;
	Font::PrintString((char const*)PSTR("KILLS:"), 2, 84, COLOUR_WHITE);

	Renderer::DrawScaled((uint16_t const*)(skeletonSpriteData + offset), 66, firstRow, 9, 255);
	Font::PrintInt(Game::stats.enemyKills[(int)EnemyType::Skeleton], 4, 84, COLOUR_WHITE);

	Renderer::DrawScaled((uint16_t const*)(mageSpriteData + offset), 66, secondRow, 9, 255);
	Font::PrintInt(Game::stats.enemyKills[(int)EnemyType::Mage], 6, 84, COLOUR_WHITE);
	Renderer::DrawScaled((uint16_t const*)(batSpriteData + offset), 96, firstRow, 9, 255, true);//òùòð»áµ¹¹Ò,invert=1
	Font::PrintInt(Game::stats.enemyKills[(int)EnemyType::Bat], 4, 114, COLOUR_WHITE);

	Renderer::DrawScaled((uint16_t const*)(spiderSpriteData + offset), 96, secondRow, 9, 255);
	...
	Font::PrintString((char const*)PSTR("FINAL SCORE:"), 7, 20, COLOUR_WHITE);
