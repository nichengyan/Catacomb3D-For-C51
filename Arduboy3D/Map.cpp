...
CellType::Value Map::GetCell(uint8_t x, uint8_t y) 
{
	...
	if(index & 1)
	{
            return (CellType::Value)(cellData >> 4);
	}
	else
	{
            return (CellType::Value)(cellData & 0xf);
	}
}

CellType::Value Map::GetCellSafe(uint8_t x, uint8_t y) 
{
...
void Map::SetCell(uint8_t x, uint8_t y, CellType::Value type)
{
	if (x >= Map::width || y >= Map::height)
	{
		return;
